...
type Event struct {
	...
	Host       Host           `json:"host"`
	AgentGuard AgentGuardData `json:"agentguard"`
}
...
func NewEvent(ftype, id, name string, score int, reasons []string, details map[string]any) Event {
	h, _ := os.Hostname()
	data := AgentGuardData{Schema: SchemaVersion{Version: "1.0.0-draft"}, Finding: Finding{ID: id, Type: ftype, Name: name, Status: "open", Confidence: 0.7}, Risk: Risk{Level: ScoreToLevel(score), Score: score, Reasons: reasons}, Details: details}
...
func SafeFingerprint(e Event) string {
	...
	sort.Strings(keys)
	chunks := []string{e.AgentGuard.Finding.Type, fmt.Sprintf("%d", e.AgentGuard.Risk.Score)}
	for _, k := range keys {
...
func ValidateRequired(e Event) error {
	if e.Event.Module == "" || e.AgentGuard.Finding.ID == "" || e.Timestamp == "" {
