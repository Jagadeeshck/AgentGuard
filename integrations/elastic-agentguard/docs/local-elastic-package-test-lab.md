...
This lab validates the AgentGuard Elastic integration without requiring the endpoint scanner. It uses checked-in NDJSON fixtures, the ingest pipeline, development Kibana references, and `elastic-package` commands only.
...
```bash
jq -r '.agentguard.finding.type' repo-root/dev-assets/sample_events/sample_events.ndjson | sort | uniq -c
```

To create a temporary pipeline fixture from a sample event, copy a single NDJSON line into `data_stream/findings/_dev/test/pipeline/test-<fixture>.log` and create a matching `test-<fixture>.log-expected.json` document following existing fixtures. Keep test values synthetic and metadata-only.
