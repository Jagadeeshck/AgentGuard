# AgentGuard Risk Scoring Model

This package accepts producer-supplied `agentguard.risk.score`, `agentguard.risk.level`, and `agentguard.risk.reasons` values and maps them into Elastic Security fields. The future endpoint scanner is responsible for calculating the score before writing NDJSON; this Elastic integration normalizes, validates, and displays the result.
...
Scores must be capped at 100 and should not be negative. Producers should keep `agentguard.risk.level` consistent with the score range.
...
- Keep `agentguard.risk.reasons` concise and normalized, for example `mcp_shell_capability`, `external_ai_api`, `temp_path_process`, or `startup_persistence`.
- Treat `agentguard.allowed: true` as triage context. It may reduce detection urgency, but it should not delete the finding.
