...
Use standalone Elastic Agent mode when a host cannot be managed by Fleet, when configuration is managed by configuration-management tooling, or when you need a minimal deployment that only reads AgentGuard NDJSON findings from disk.
...
- Expected data stream: `logs-agentguard.findings-default`
- Package ingest pipeline: `logs-agentguard.findings-default`
...
```bash
GET _ingest/pipeline/logs-agentguard.findings*
```

or in Kibana: **Stack Management → Ingest Pipelines**, then search for `logs-agentguard.findings`.
...
inputs:
  - id: agentguard-agentguard-findings
    type: filestream
    streams:
      - id: agentguard-agentguard-findings-stream
        data_stream:
          type: logs
...
Prefer API key authentication for standalone agents. Create an API key with privileges to write to `logs-agentguard.findings-*` and read package assets required by your operational model, then provide it through an environment variable or your secret-management system:
...
4. In Kibana Discover, select `logs-agentguard.findings-*` and filter for `event.dataset: agentguard.findings`.
...
- **Pipeline not found**: install or reinstall the package assets, then search ingest pipelines for `logs-agentguard.findings*`.
...
- **Invalid JSON**: check events tagged `agentguard_invalid_json`; each line must be valid NDJSON.
