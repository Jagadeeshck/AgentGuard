...
This document defines production-style Elastic Security detection rules for AgentGuard findings.
...
- `type`: `query`
- `language`: `kuery`
- `index`: `["logs-agentguard.findings-*"]`
- `interval`: `5m`
- `from`: `now-10m`
- `enabled`: `false`
- `author`: `["AgentGuard"]`
- `tags`:
  ...
  - `Elastic Security`
- `references`:
  - `https://github.com/Jagadeeshck/integrations/elastic-agentguard#readme`

## Rule definitions

### 1) AgentGuard Critical Finding

- **Purpose**: Detects any critical AgentGuard finding.
- **KQL**: `event.module: "agentguard" and agentguard.risk.level: "critical"`
- **Severity**: `critical`
- **Risk score**: `90`
- **Expected false positives**: High-risk but approved internal red-team or security engineering simulations.
- **Tuning guidance**: Mark known-approved workflows with `agentguard.allowed: true` to suppress expected events.
- **Sample event type**: `agentguard.risk.level: "critical"`
...
- **KQL**: `event.module: "agentguard" and agentguard.finding.type: "mcp_server" and not agentguard.allowed: true and agentguard.mcp.capabilities: ("shell" or "filesystem")`
- **Severity**: `high`
- **Risk score**: `80`
- **Expected false positives**: Known MCP servers used by approved internal AI tooling.
- **Tuning guidance**: Allowlist known MCP config paths and set trusted MCP findings to `agentguard.allowed: true`.
- **Sample event type**: `agentguard.finding.type: "mcp_server"`

### 3) AI Agent With Shell Tool Use

- **Purpose**: Detects untrusted AI agent shell execution behavior.
- **KQL**: `event.module: "agentguard" and agentguard.finding.type: "ai_agent_shell_tool_use" and not agentguard.allowed: true`
- **Severity**: `high`
- **Risk score**: `82`
- **Expected false positives**: Approved automation agents that legitimately execute shell commands.
- **Tuning guidance**: Allowlist approved process executable paths and automation identities; mark expected behavior with `agentguard.allowed: true`.
- **Sample event type**: `agentguard.finding.type: "ai_agent_shell_tool_use"`

### 4) Browser AI Extension With Broad Permissions

- **Purpose**: Detects AI browser extensions with broad or sensitive permissions.
- **KQL**: `event.module: "agentguard" and agentguard.finding.type: "browser_extension" and not agentguard.allowed: true and agentguard.extension.permissions: ("<all_urls>" or "scripting" or "webRequest" or "clipboardRead" or "nativeMessaging")`
- **Severity**: `medium`
- **Risk score**: `65`
...
- **Sample event type**: `agentguard.finding.type: "browser_extension"`

### 5) Local LLM Service Exposed Beyond Loopback

- **Purpose**: Detects local AI/LLM services exposed beyond loopback interfaces.
- **KQL**: `event.module: "agentguard" and agentguard.finding.type: "local_llm_service" and agentguard.ai.local_service: true and not destination.ip: ("127.0.0.1" or "::1" or "localhost")`
- **Severity**: `high`
- **Risk score**: `78`
...
- **Sample event type**: `agentguard.finding.type: "local_llm_service"`

### 6) AI Tool Added to Startup

- **Purpose**: Detects untrusted persistence behavior via startup registration.
- **KQL**: `event.module: "agentguard" and agentguard.finding.type: "startup_item" and not agentguard.allowed: true`
- **Severity**: `medium`
- **Risk score**: `60`
- **Expected false positives**: Approved internal AI tooling installed for managed persistence.
- **Tuning guidance**: Allowlist approved process executable paths and startup locations; mark known-good with `agentguard.allowed: true`.
- **Sample event type**: `agentguard.finding.type: "startup_item"`
...
- **KQL**: `event.module: "agentguard" and agentguard.finding.type: ("ai_cyber_agent_activity" or "ai_vulnerability_research_agent" or "ai_exploit_development_activity" or "ai_sandbox_escape_research") and not agentguard.allowed: true and agentguard.risk.level: ("high" or "critical")`
- **Severity**: `critical`
- **Risk score**: `92`
- **Expected false positives**: Approved security researcher activity and sanctioned purple-team exercises.
- **Tuning guidance**: Mark sanctioned research with `agentguard.allowed: true` and scope additional environment-based allowlists.
- **Sample event type**: `agentguard.finding.type: "ai_cyber_agent_activity"`

### 8) AI Agent Scanning Sensitive Repositories

- **Purpose**: Detects untrusted AI agent scanning of sensitive repositories.
- **KQL**: `event.module: "agentguard" and agentguard.finding.type: "ai_agent_sensitive_repo_scan" and not agentguard.allowed: true`
- **Severity**: `high`
- **Risk score**: `84`
- **Expected false positives**: Approved code indexing and SAST automation.
- **Tuning guidance**: Allowlist approved automation and repository scanners; mark expected scans with `agentguard.allowed: true`.
- **Sample event type**: `agentguard.finding.type: "ai_agent_sensitive_repo_scan"`

### 9) AI Agent Running Security Tools

- **Purpose**: Detects untrusted AI agents orchestrating security tooling.
- **KQL**: `event.module: "agentguard" and agentguard.cyber_agent.security_tools: * and not agentguard.allowed: true`
- **Severity**: `high`
- **Risk score**: `85`
...
- **Sample event type**: `agentguard.cyber_agent.security_tools: *`
...
- **KQL**: `event.module: "agentguard" and tags: "agentguard_pipeline_failure"`
- **Severity**: `medium`
- **Risk score**: `50`
...
- **Sample event type**: `tags: "agentguard_pipeline_failure"`
...
- Use `agentguard.allowed: true` for known-good activity.
