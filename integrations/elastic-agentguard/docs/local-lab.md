...
Use this lab to validate the AgentGuard Elastic integration before an AgentGuard endpoint scanner exists. The lab exercises only the Elastic integration package and synthetic NDJSON findings; it does not include endpoint scanner logic.
...
The validation corpus lives in `repo-root/dev-assets/test_data/`. Each file contains one or more synthetic JSON lines matching the `agentguard.*` schema.
...
while IFS= read -r line; do
  curl -k -u "$ES_USER:$ES_PASS" \
    ...
    -X POST "$ES_URL/logs-agentguard.findings-default/_doc" \
    -d "$line"
...
```bash
curl -k -u "$ES_USER:$ES_PASS" \
  "$ES_URL/logs-agentguard.findings-default/_search?pretty" \
  -H 'Content-Type: application/json' \
  -d '{"size":5,"sort":[{"@timestamp":"desc"}],"query":{"term":{"event.dataset":"agentguard.findings"}}}'
```

In Kibana Discover, select the data view that includes `logs-agentguard.findings-*` and confirm events appear with:
...
1. Open Kibana and confirm the AgentGuard integration package is installed.
