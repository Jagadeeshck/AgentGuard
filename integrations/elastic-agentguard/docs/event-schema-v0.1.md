# AgentGuard finding event schema v0.1

This document defines the minimum event contract consumed by the AgentGuard Elastic integration package. The package ingests newline-delimited JSON findings emitted by a separate AgentGuard endpoint product; it does not implement endpoint scanning.
...
All non-ECS custom fields must live below `agentguard.*`. ECS fields such as `host.*`, `process.*`, `user.*`, `file.*`, `source.*`, `destination.*`, `network.*`, `dns.*`, `event.*`, `observer.*`, and `related.*` may be used when they match ECS semantics.

## Data stream

- Type: `logs`
- Dataset: `agentguard.findings`
- Default data stream: `logs-agentguard.findings-default`
...
| `agentguard.finding.id` | keyword | Yes | Stable finding identifier. |
| `agentguard.finding.type` | keyword | Yes | Finding family. Supported values include `ai_api_connection`, `mcp_server`, `browser_extension`, `startup_item`, `local_llm_service`, `suspicious_agent_process`, `mcp_config_modified`, and the AI cyber-agent detection pack values: `ai_cyber_agent_activity`, `ai_vulnerability_research_agent`, `ai_sandbox_escape_research`, `ai_fuzzing_activity`, `ai_reverse_engineering_activity`, `ai_exploit_development_activity`, `ai_security_tool_mcp_server`, `ai_agent_shell_tool_use`, `ai_agent_sensitive_repo_scan`, and `ai_agent_mass_codebase_analysis`. |
| `agentguard.risk.level` | keyword | Yes | One of `low`, `medium`, `high`, or `critical`. |
| `agentguard.risk.score` | float | Yes | Numeric risk score, typically 0-100. |
| `agentguard.allowed` | boolean | Recommended | Whether the finding is allowlisted or approved by policy. |
...
The pipeline adds `event.module`, `event.dataset`, `observer.*`, AgentGuard tags, `related.*`, and derived event categorization fields.
...
- `agentguard.ai.endpoint` with query-string secrets removed or redacted
- `agentguard.ai.local_service: false`
- `agentguard.ai.confidence`
...
- `agentguard.mcp.client.name`
- `agentguard.mcp.server.name`
- `agentguard.mcp.server.command` with secrets redacted
- `agentguard.mcp.server.args` with secrets redacted
- `agentguard.mcp.config.path`
- `agentguard.mcp.tools`
- `agentguard.mcp.capabilities`
- `agentguard.mcp.risk_reasons`
...
- `agentguard.browser.name`
- `agentguard.browser.profile`
- `agentguard.extension.id`
- `agentguard.extension.name`
- `agentguard.extension.version`
- `agentguard.extension.permissions`
- `agentguard.extension.host_permissions`
- `agentguard.extension.risk_reasons`
...
- `agentguard.startup.type`
- `agentguard.startup.name`
- `agentguard.startup.path`
- `agentguard.startup.command` with secrets redacted
- `agentguard.startup.enabled`
...
- `agentguard.ai.provider`
- `agentguard.ai.endpoint`
- `agentguard.ai.local_service: true`
- `agentguard.ai.model_hint` if it does not reveal prompt or response content
- ECS `process.*`, `destination.*`, and `network.*`
...
The cyber-agent pack detects Mythos-like AI cyber-agent activity using behaviour-based metadata. The integration must not collect prompt content, secrets, decrypted traffic, clipboard content, browsing history, exploit logic, or offensive payloads. The optional word `mythos` may appear as one weak indicator in `agentguard.cyber_agent.suspicious_keywords`, but producers and rules must combine it with concrete behaviours such as shell access, filesystem access, security tooling, sensitive path scanning, or exploit-development file metadata.
...
Recommended fields under `agentguard.cyber_agent.*`:
...
The ingest pipeline copies `agentguard.risk.score` to `event.risk_score` and derives `event.severity` from `agentguard.risk.level`:
