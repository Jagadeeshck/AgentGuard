...
| `repo-root/dev-assets/test_data/startup-item-ai-agent.ndjson` | `startup_item` | `agentguard_critical_finding.toml` | critical | no | High severity only; should not match critical catch-all. |
...
| `repo-root/dev-assets/test_data/exploit-like-file-write.ndjson` | `ai_exploit_development_activity` | `agentguard_critical_finding.toml` | critical | yes | Score is in the critical range. |
